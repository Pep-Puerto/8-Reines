...
X_dib_enreixat(10,5,n);
ubica_posyx(10,5,n,&cA);

//case principal
switch(op){
	case 1: //Calcula autom�icament
		cal_cumagic(n,n*n);
		X_imp_cumagic(n,&cA);
		break;
	...
	case 3: //Resoluci amb el ratol
		...
		X_dib_enreixat(10,(n+3)*AMP_C,n);
		ubica_posyx(10,(n+3)*AMP_C,n,&cB);
		cal_emplenar_cB(n);
		X_imp_cumagic(n,&cB);
		//Activa els events del mouse
		X_act_event_mouse();
		while(quit=!1){
			TipusEvent=CapturaEvent(&x,&y);
			switch(TipusEvent){
			  case ButtonPress:
...
int y,x;
	...
	x=e->pos_erx.x;
	X_printf(y,x,"Fila:    Columna:   Valor:  ");
	X_printf(y,x,"Fila: %d  Columna: %d Valor:%d");
}
